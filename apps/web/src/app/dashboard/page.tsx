...
export default function DashboardPage() {
  const [filter, setFilter] = useState<BookingState | "future" | "past">("future")
  ...
  useEffect(() => {
    async function fetchBookings() {
      const data = await getUserBookings('cm6ry32iu0000xz076yvedk6k')
      setBookings(data)
    }
    fetchBookings()
  }, [])
  ...
  const filteredBookings = bookings
    .filter((booking) => {
      ...
      if (filter === "pending") return booking.state === BookingState.CONTATTARE || booking.state === BookingState.CONTATTATO
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {filteredBookings.map((booking) => (
                <BookingCard key={booking.userId + booking.start} booking={booking} />
              ))}
            </div>
          </div>
